...
import wandb
from src.Baseline import llm_rerank, cross_encoder
...
def main(dataset_name, model_name, top_k_passages, args):
    ...
    print("\n")
    results = {}
    for query, q_id, query_embedding in tqdm(zip(queries, query_ids, query_embeddings), desc=" > CE Reranking", total=len(query_ids)):
        pred, _ = cross_encoder(
            query=query,
...
def arg_parser():
    parser = argparse.ArgumentParser(description='Reranking with CE')
