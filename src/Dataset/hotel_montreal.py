...
class HotelMontreal:
    def load_dataset(self, cache_path="data/hotel_montreal/cache.csv"):
        ...
        for city_id, file in enumerate(file_names):
            ...
            with open(f"data/hotel_montreal/corpus/{file}", "r", encoding='utf-8') as f:
        ...
        # Step 4: Load queries
        with open(f"data/hotel_montreal/queries.txt", "r", encoding='utf-8', errors='ignore') as f:
            queries = f.read().strip().split('\n')

        # Step 5: Load ground truth directly from JSON
        with open(f"data/hotel_montreal/ground_truth.json", "r", encoding='utf-8', errors='ignore') as f:
