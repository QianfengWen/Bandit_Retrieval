...
class HotelLondon:
    def load_dataset(self, cache_path="data/hotel_london/cache.csv"):
        self.cache_path = cache_path

        # Step 1: Read file names and sort them
        file_names = sorted(os.listdir("data/hotel_london/corpus"))
        ...
        for city_id, file in enumerate(file_names):
            ...
            with open(f"data/hotel_london/corpus/{file}", "r", encoding='cp1252') as f:
        ...
        # Step 4: Load queries
        with open(f"data/hotel_london/queries.txt", "r", encoding='cp1252', errors='ignore') as f:
            queries = f.read().strip().split('\n')

        # Step 5: Load ground truth directly from JSON
        with open(f"data/hotel_london/ground_truth.json", "r", encoding='cp1252', errors='ignore') as f:
