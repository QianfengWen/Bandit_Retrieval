...
class HotelLondon:
    def load_dataset(self, cache_path="data/hotel_london/cache.csv"):
        ...
        for city_id, file in enumerate(file_names):
            ...
            with open(f"data/hotel_london/corpus/{file}", "r", encoding='utf-8') as f:
        ...
        # Step 4: Load queries
        with open(f"data/hotel_london/queries.txt", "r", encoding='utf-8', errors='ignore') as f:
            queries = f.read().strip().split('\n')

        # Step 5: Load ground truth directly from JSON
        with open(f"data/hotel_london/ground_truth.json", "r", encoding='utf-8', errors='ignore') as f:
