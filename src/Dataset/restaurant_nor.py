...
class RestaurantNor:
    def load_dataset(self, cache_path="data/restaurant_nor/cache.csv"):
        self.cache_path = cache_path

        # Step 1: Read file names and sort them
        file_names = sorted(os.listdir("data/restaurant_nor/corpus"))
        ...
        for city_id, file in enumerate(file_names):
            ...
            with open(f"data/restaurant_nor/corpus/{file}", "r", encoding='utf-8') as f:
        ...
        # Step 4: Load queries
        with open(f"data/restaurant_nor/queries.txt", "r", encoding='utf-8') as f:
            queries = f.read().strip().split('\n')

        # Step 5: Load ground truth directly from JSON
        with open(f"data/restaurant_nor/ground_truth.json", "r", encoding='utf-8') as f:
